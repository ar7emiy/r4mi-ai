...
; (function () {
  ...
  // ── 1. Inject element_resolver.js + capture.js ────────────────────────────
  // element_resolver.js exposes window.r4mi.resolve(fingerprint) so r4mi can
  // find DOM elements without testids — site-agnostic, accessibility-first.
  var resolverScript = document.createElement('script')
  resolverScript.src = ORIGIN + '/element_resolver.js'
  ...
  window.addEventListener('message', function (e) {
    ...
    switch (msg.type) {
      ...
      case 'r4mi:demo-step':
        // Universal element resolution via element_resolver.js — no hardcoded
        // testids, no domain-specific routing. The step carries an
        // ElementFingerprint (target_fingerprint) generated by capture.js at
        // teach time; we resolve it against the current DOM and animate the
        // value in.
        if (msg.step) {
          var step = msg.step
          var fp = step.target_fingerprint || null
          var resolved = (window.r4mi && window.r4mi.resolve)
            ? window.r4mi.resolve(fp)
            : { element: null, confidence: 0 }
          var input = resolved.element

          if (!input) {
            // No DOM target — surface the step as a window event so the host
            // app can react if it wants (e.g., HITLReplay still sees the
            // descriptive payload).
          ...
          var isFormInput = (input.nodeName === 'INPUT' || input.nodeName === 'TEXTAREA' || input.nodeName === 'SELECT')
          var isReadOnly = !isFormInput || input.disabled || input.readOnly
          ...
          if (isReadOnly) {
            ...
            var rect2 = input.getBoundingClientRect()
            label.style.left = (rect2.left + window.scrollX) + 'px'
            label.style.top = (rect2.top + window.scrollY) + 'px'
            document.body.appendChild(label)
      ...
      case 'r4mi:resolve-and-fetch':
        // The agent wants to replay a captured fetch through the host's
        // origin so cookies/auth carry over. Returns the response body to
        // the requesting iframe via postMessage.
        if (msg.method && msg.url) {
          var requestId = msg.request_id || ('r4mi-' + Date.now())
          var fetchOpts = { method: msg.method, headers: { 'Content-Type': 'application/json' } }
          if (msg.body != null) {
            try { fetchOpts.body = typeof msg.body === 'string' ? msg.body : JSON.stringify(msg.body) }
            catch (_) { fetchOpts.body = String(msg.body) }
          }
