...
; (function () {
  ...
  // ── 5. Field-fill helper (shared by SSE AGENT_DEMO_STEP and postMessage r4mi:demo-step) ──
  ...
  // ── 5b. SSE listener for badge + agent field fills ──────────────────────────
  var BADGE_EVENTS = [
  ...
  try {
    ...
    sse.onmessage = function (e) {
      try {
        ...
        // Agent run step — resolve fingerprint and fill the field on the host page
        if (data.event === 'AGENT_DEMO_STEP') {
          var step = data.data || {}
          fillField(step)
        }
      } catch (_) { }
  ...
  window.addEventListener('message', function (e) {
    ...
    switch (msg.type) {
      ...
      case 'r4mi:demo-step':
        // Universal element resolution via element_resolver.js.
        // Delegates to fillField() which is also used for SSE AGENT_DEMO_STEP.
        if (msg.step) {
          fillField(msg.step)
        }
        break
